...
FtpStations::FtpStations(Progress *pro) : ftp()
{
  URL = strdup(profile->ftp_addr);
  Username = strdup(profile->ftp_user);
  Password = strdup(profile->ftp_pw);
  Path = strdup(profile->ftp_download_path);
  this->pro = pro;
}
FtpStations::~FtpStations()
{
	free(URL);
  free(Username);
  free(Password);
...
bool FtpStations::Login()
{
	int ret;
  char buf[64];
  // Open up ftp
  pro->SetText("Open FTP");
  ret = DoOpen(URL);
  if(ret) {
  	pro->SetText("FTP Problem connection to %s",URL);
   	//MdiClient->MessageBox(buf);
  	DoClose();
    return true;
  }
  ret = DoLogin(Username,Password);
  pro->SetText("FTP opened. Logging in with name: %s",Username);
  if(ret) {
  	pro->SetText("FTP problem logging in as user \"%s\" and pw \"%s\"",
      Username,Password);
  	//MdiClient->MessageBox(buf);
  	DoClose();
    return true;
  ...
  if(DoCD(Path)) {
  	...
  	pro->SetText("FTP problem mvoing to directory \"%s\"",Path);
 		//MdiClient->MessageBox(buf);
   	DoClose();
 		return true;
  }
  pro->SetText("Fetch \"IRISEdStations.xml\"");
  DoGet("IRISEdStations.xml");
  return false;
}
...
int ftp::ReadControlMsg( char *szBuffer, int len)
{
  ...
  if(IsReadyToRead(hControlSocket)) {
  	if( (ret=recv(hControlSocket,szBuffer,len,0)) < 0) {
    	//pro->SetText("Socket problem in recv/ReadControlMsg: %d",ret);
      GetError();
    }
  } else {
  	//pro->SetText("Not ready to read in ReadControlMsg");
  }
  return ret;
}
...
int ftp::ReadDataMsg( char *szBuffer, int len)
{
  ...
  if(IsReadyToRead(hDataSocket)) {
   	if( (ret=recv(hDataSocket,szBuffer,len,0)) < 0) {
  		//pro->SetText("Socket problem in recv/ReadDataMsg: %d",ret);
      GetError();
    }
  } else {
  	//pro->SetText("Not ready to read in ReadDataMsg");
  }
  return ret;
}
...
bool ftp::DoPutData( char *fname, char *data, int len)
{
  ...
  */
  ...
  if(strncmp(szBuffer,"226",3)) {
  	//pro->SetText("Error in DoPutData: %s",szBuffer);
    return true;
  }
  ret = false;
  return ret;
    ...
    timeTimeOut.tv_sec = 5;	// Timeout after 5 seconds
		...
		if ((nReady = select(NULL, (LPFD_SET)&setReadyToRead, NULL, NULL,
					&timeTimeOut)) == SOCKET_ERROR)
			{
				...
				//pro->SetText("%s Function: %s",szBuffer, lpszFunctionName);
				return(FALSE);
			}
