...
#include "as1.h"
extern Profile *profile;  //Roger added this definition
...
PSN::PSN(KSAC *ksac) {
	PHdr = new PsnHeader;
   //oldfmt = new oldFmt;     //Roger added this line
   newfmt = new newFmt;     //Roger added this line
  Initiate();           //Roger  Function Initiate identifies this as a PSN format 3 file
  ...
  //PHdr->latDec = (short)(((int)(0.5+100*ksac->GetSACHV(STLA)))%100); //Original code
   PHdr->latDec = (short)10*((int)(100*ksac->GetSACHV(STLA))%100);  //Roger tried, works

   
  PHdr->longInt = (short)ksac->GetSACHV(STLO);
  //PHdr->longDec =(short)(((int)(0.5+100*ksac->GetSACHV(STLO)))%100); //Original code
  PHdr->longDec =(short)10*((int)(1-100*ksac->GetSACHV(STLO))%100); //Roger tried, worked

   //Roger- zero these structure data items
  PHdr->u.newfmt.adcType,PHdr->u.newfmt.adcNull,PHdr->u.newfmt.adcMin,PHdr->u.newfmt.adcMax = 0;
  PHdr->u.newfmt.numConv = 0;
  PHdr->u.newfmt.magCorrInt, PHdr->u.newfmt.magCorrDec = 0;
  PHdr->u.newfmt.goodTime, PHdr->u.newfmt.neicHour = 0;
  PHdr->u.newfmt.neicDay, PHdr->u.newfmt.neicMonth = 0;
  PHdr->u.newfmt.msPeriod, PHdr->u.newfmt.mbPeriod = 0;
  PHdr->u.newfmt.magType[2] = 0;
  PHdr->u.newfmt.magnitude = 0;
  PHdr->u.newfmt.depth = 0;
  PHdr->u.newfmt.quakeLatInt, PHdr->u.newfmt.quakeLatDec = 0;
  PHdr->u.newfmt.quakeLongInt, PHdr->u.newfmt.quakeLongDec = 0;
   PHdr->u.newfmt.pPickTime = 0;           //Roger-worked!
   PHdr->u.newfmt.sPickTime = 0;           //Roger-worked!
   PHdr->u.newfmt.pYLocation = 0;          //Roger-worked!
   PHdr->u.newfmt.sYLocation = 0;          //Roger-worked!
   PHdr->u.newfmt.pickTableInfo = 0;
   PHdr->u.newfmt.lockChr, PHdr->u.newfmt.bSpare = 0;
   PHdr->u.newfmt.mdSec = 0;
   PHdr->u.newfmt.magCorrMult, PHdr->u.newfmt.mbCorrNum = 0;
   PHdr->u.newfmt.spare1[2] = 0;
   saveIntStringNew(PHdr->u.newfmt.comment, "(Default)", 30);    //Roger--worked!
   PHdr->neicFlag = 0;                     //Roger
   PHdr->neicMin = 0;                      //Roger
   PHdr->neicSec = 0;                      //Roger
   PHdr->neic100th = 0;                    //Roger
   //saveIntString(PHdr->location, "Ellensburg, Wa.", 15);    //Roger-worked
    saveIntString(PHdr->location, profile->station_name, 15);    //Roger
   PHdr->orientation =(short) profile->component;    //Roger- works
...
PSN::~PSN(){
  delete[] PPSNData;
  delete PHdr;
  //delete oldfmt;     //Roger added
  //delete newfmt;    //Roger added
}
...
//-Write PSN file
//
short PSN::WritePSN(char* PPSNFile)         //Roger  This is called from "event"
{
	if(PPSNFile != 0)
  	strcpy(filename,PPSNFile);
	ofstream PSNStrm(filename,ios::binary);                //Original code
   //ofstream PSNStrm(PPSNFile,ios::trunc | ios::binary);    //Roger added trying to zero saved file prior to writing

  PSNStrm.write((unsigned char *) bsave,7);
  PSNStrm.write((unsigned char *) PHdr,sizeof(PsnHeader));
  //PSNStrm.write((unsigned char *) newfmt,sizeof(newFmt));    //Roger tried to initialize string
  //PSNStrm.write((unsigned char *) "a long test string",100);                   //Roger tried writing header in first 100 bytes
  // Convert to short
